...
class Image {
  ...
  function get($url, &$pipeline) {
	...
	// at all
	//
	$filename = (new Image())->make_cache_filename($url);
	...
	if(empty($data->content)){ 
		$str_file = str_replace("http://www.druckerei-verwaltung.de", PATH . "/htdocs_admin", $url); 
		
		$handle = fopen($str_file, 'r');
		$data->content = fread($handle, filesize($str_file));
		fclose($handle);
	}
	#echo "<pre>";
	#print_r($data);
