...
class CSSBackgroundPosition extends CSSSubFieldProperty {
  ...
  function build_value($x, $y) {
    return array((new CSSBackgroundPosition())->build_subvalue($x),
                 (new CSSBackgroundPosition())->build_subvalue($y));
  }
  ...
  function parse_in($value) {
    if (preg_match("/(".LENGTH_REGEXP."|".PERCENTAGE_REGEXP."|".TEXT_REGEXP."|\b0\b)\s+(".LENGTH_REGEXP."|".PERCENTAGE_REGEXP."|".TEXT_REGEXP."|\b0\b)/", $value, $matches)) {
      ...
      $type_x = (new CSSBackgroundPosition())->detect_type($x);
      $type_y = (new CSSBackgroundPosition())->detect_type($y);

      if (is_null($type_x) && is_null($type_y)) {
        return (new CSSBackgroundPosition())->build_value($x,$y);
      }
      ...
      return (new CSSBackgroundPosition())->build_value($y,$x);
    }
    ...
    if (preg_match("/".LENGTH_REGEXP."|".PERCENTAGE_REGEXP."/", $value, $matches)) {
      $x = $matches[0];
      return (new CSSBackgroundPosition())->build_value($x,"50%");
    }
  ...
  function parse($value) {
    ...
    $value = (new CSSBackgroundPosition())->parse_in($value);
    return new BackgroundPosition($value[0][0] ?? 0, $value[0][1] ?? 0,
