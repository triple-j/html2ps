...
class CSSBorderWidth extends CSSSubProperty {
  ...
  function __construct(&$owner) {
    ...
    $this->_defaultValue = new BorderWidth((new Value())->fromData(0, UNIT_PT),
                                           (new Value())->fromData(0, UNIT_PT),
                                           (new Value())->fromData(0, UNIT_PT),
                                           (new Value())->fromData(0, UNIT_PT));
  }
  ...
  function parse_value($value) {
    ...
    case 'thin':
      return (new Value())->fromString('1px');
    case 'medium':
      return (new Value())->fromString('3px');
    case 'thick':
      return (new Value())->fromString('5px');
    default:
