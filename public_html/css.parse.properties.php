...
function &parse_css_properties($string, &$pipeline) {
  ...
  while ($string != '') {
    ...
    $property =& (new CSSPropertyDeclaration())->create($code, $value, $pipeline);
    if (!is_null($property)) {
...
function parse_css_properties_property($string, &$code) {
  $identifier_regexp = (new CSS())->get_identifier_regexp();
...
function parse_css_properties_value($string, &$value) {
  ...
  do {
    ...
    list($new_value, $string) = (new CSS())->parse_string($string);
