<?php
// $Header: /cvsroot/html2ps/html.attrs.inc.php,v 1.62 2007/02/18 09:55:11 Konstantin Exp $
...
function attr_name_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_HTML2PS_LINK_DESTINATION);
...
function attr_href_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_HTML2PS_LINK_TARGET);
...
function attr_frameborder_before(&$root, &$pipeline) {
  ...
  $handler =& (new CSS())->get_handler(CSS_BORDER);
...
function attr_iframe_marginheight_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_PADDING_TOP);
  $handler->css((int)$root->get_attribute('marginheight').'px',$pipeline);
  $handler =& (new CSS())->get_handler(CSS_PADDING_BOTTOM);
  $handler->css((int)$root->get_attribute('marginheight').'px',$pipeline);
}
...
function attr_iframe_marginwidth_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_PADDING_RIGHT);
  $handler->css((int)$root->get_attribute('marginwidth').'px',$pipeline);
  $handler =& (new CSS())->get_handler(CSS_PADDING_LEFT);
  $handler->css((int)$root->get_attribute('marginwidth').'px',$pipeline);
}
...
function attr_body_text_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_COLOR);
...
function attr_body_link_before(&$root, &$pipeline) {
  ...
  $collection->addProperty((new CSSPropertyDeclaration())->create(CSS_COLOR, $color, $pipeline));
...
function attr_body_topmargin_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_MARGIN_TOP);
...
function attr_body_leftmargin_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_MARGIN_LEFT);
...
function attr_body_marginheight_before(&$root, &$pipeline) {
  ...
  $h_top    =& (new CSS())->get_handler(CSS_MARGIN_TOP);
  $h_bottom =& (new CSS())->get_handler(CSS_MARGIN_BOTTOM);
...
function attr_body_marginwidth_before(&$root, &$pipeline) {
  ...
  $h_left  =& (new CSS())->get_handler(CSS_MARGIN_LEFT);
  $h_right =& (new CSS())->get_handler(CSS_MARGIN_RIGHT);
...
function attr_background_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_BACKGROUND_IMAGE);
...
function attr_table_float_align_after_styles(&$root, &$pipeline) {
  if ($root->get_attribute('align') === 'center') {     
    $margin_left =& (new CSS())->get_handler(CSS_MARGIN_LEFT);
    $margin_left->css('auto',$pipeline);
    
    $margin_right =& (new CSS())->get_handler(CSS_MARGIN_RIGHT);
    $margin_right->css('auto',$pipeline);
  } else {
    $float =& (new CSS())->get_handler(CSS_FLOAT);
    $css_state =& $pipeline->getCurrentCSSState();
...
function attr_self_align_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_HTML2PS_LOCALALIGN);
  $css_state =& $pipeline->getCurrentCSSState();
...
function attr_border_before(&$root, &$pipeline) {
  ...
  $border->left->width   = (new Value())->fromData($width, UNIT_PX);
  $border->right->width  = (new Value())->fromData($width, 'px');
  $border->top->width    = (new Value())->fromData($width, 'px');
  $border->bottom->width = (new Value())->fromData($width, 'px');
...
function attr_table_border_before(&$root, &$pipeline) {
  ...
  $border->left->width   = (new Value())->fromData($width, UNIT_PX);
  $border->right->width  = (new Value())->fromData($width, UNIT_PX);
  $border->top->width    = (new Value())->fromData($width, UNIT_PX);
  $border->bottom->width = (new Value())->fromData($width, UNIT_PX);
...
function attr_width_before(&$root, &$pipeline) {
  $width =& (new CSS())->get_handler(CSS_WIDTH);
...
function attr_height_required_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_HEIGHT);
...
function attr_height_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_MIN_HEIGHT);
...
function attr_font_size_before(&$root, &$pipeline) {
  ...
  $handler =& (new CSS())->get_handler(CSS_FONT_SIZE);
...
function attr_font_face_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_FONT_FAMILY);
...
function attr_input_name_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_HTML2PS_FORM_RADIOGROUP);
...
function attr_input_size_before(&$root, &$pipeline) {
  ...
  case 'text':
  case 'password':
    $handler =& (new CSS())->get_handler(CSS_WIDTH);
...
function attr_cellspacing_before(&$root, &$pipeline) {
  $css_state =& $pipeline->getCurrentCSSState();
  $handler =& (new CSS())->get_handler(CSS_HTML2PS_CELLSPACING);
  $handler->replace((new Value())->fromData((int)$root->get_attribute('cellspacing'), UNIT_PX),
                    $css_state);
}
...
function attr_cellpadding_before(&$root, &$pipeline) {
  $css_state =& $pipeline->getCurrentCSSState();
  $handler =& (new CSS())->get_handler(CSS_HTML2PS_CELLPADDING);
  $handler->replace((new Value())->fromData((int)$root->get_attribute('cellpadding'), UNIT_PX),
                    $css_state);
}
...
function attr_start_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_HTML2PS_LIST_COUNTER);
  $css_state =& $pipeline->getCurrentCSSState();
...
function attr_hr_color_before(&$root, &$pipeline) {
  $handler =& (new CSS())->get_handler(CSS_BORDER_COLOR);
  $handler->css($root->get_attribute('color'), $pipeline); 
}
