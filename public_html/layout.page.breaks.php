...
class PageBreakLocator {
  function _getBreakLocations(&$dom_tree) {
    $locations_ungrouped = (new PageBreakLocator())->_getPagesTraverse($dom_tree, 0);

    /**
     * If there's no page break locations (e.g. document is empty)
     * generate one full-size page
     */
    if (is_countable($locations_ungrouped) && count($locations_ungrouped) == 0) {
      return array();
    }

    return (new PageBreakLocator())->sort_locations($locations_ungrouped);
  }
  ...
  function _getFootnotesTraverse(&$box) {
    ...
    if (is_a($box, "BoxNoteCall")) {
      ...
      foreach ($box->content as $child) {
        $footnotes = array_merge($footnotes, (new PageBreakLocator())->_getFootnotesTraverse($child));
      }
    }
  ...
  function getPages(&$dom_tree, $max_page_height, $first_page_top) {
     ...
     */
    $footnotes = (new PageBreakLocator())->_getFootnotesTraverse($dom_tree);
    usort($footnotes, 'cmp_footnote_locations');

    $locations = (new PageBreakLocator())->_getBreakLocations($dom_tree);
  ...
  function _hasForcedPageBreakBefore(&$box) {
    return (new PageBreakLocator())->_isForcedPageBreak($box->getCSSProperty(CSS_PAGE_BREAK_BEFORE));
  }

  function _hasForcedPageBreakAfter(&$box) {
    return (new PageBreakLocator())->_isForcedPageBreak($box->getCSSProperty(CSS_PAGE_BREAK_AFTER));
  }
  ...
  function _getPagesTraverseBlock(&$box, &$next, &$previous, $penalty) {
     ...
     * Process box content
     */
    $locations = array_merge($locations, (new PageBreakLocator())->_getPagesTraverse($box, $penalty));
  ...
  function _getPagesTraverseTableRow(&$box, $penalty) {
    ...
    if (is_countable($cells) && count($cells) > 0) {
    ...
    $cells = $box->getChildNodes();
    $null = null;
    $ungrouped_row_locations = (new PageBreakLocator())->_getPagesTraverseBlock($cells[0], 
                                                                       $null, 
                                                                       $null, 
                                                                       $inside_penalty);
    $row_locations = (new PageBreakLocator())->sort_locations($ungrouped_row_locations);

    for ($i=1, $size = count((array) $cells); $i < $size; $i++) {
      $ungrouped_child_locations = (new PageBreakLocator())->_getPagesTraverseBlock($cells[$i], 
                                                                            $null, 
                                                                            $null, 
                                                                            $inside_penalty);
      $child_locations = (new PageBreakLocator())->sort_locations($ungrouped_child_locations);

      $new_row_locations = array();

      $current_row_location_index = 0;
      while ($current_row_location_index < count((array) $row_locations)) {
        $current_row_location = $row_locations[$current_row_location_index];
        for ($current_child_location_index = 0, $child_locations_total = count((array) $child_locations);
  ...
  function _getPagesTraverse(&$box, $penalty) {
    ...
    for ($i=0, $content_size = count((array) $box->content); $i<$content_size; $i++) {
      $previous_child =& (new PageBreakLocator())->_getPrevious($i, $box->content, $content_size);
      $next_child     =& (new PageBreakLocator())->_getNext($i, $box->content, $content_size);
      $child          =& $box->content[$i];
      ...
      if (is_a($child, "BRBox")) {
        ...
        $locations = array_merge($locations, (new PageBreakLocator())->_getPagesTraverseBlock($child, 
                                                                                      $next_child,
                                                                                      $previous_child,
                                                                                      $penalty));
      } elseif (is_a($child, "TableCellBox")) {
        $null = null;
        $child_locations = (new PageBreakLocator())->_getPagesTraverseBlock($child, $null, $null, $penalty);
        ...
        if (is_a($previous_child, "BRBox")) {
          $more_before = (new PageBreakLocator())->_getMoreBefore($i, $box->content, $content_size);
        }

        if (is_a($next_child, "BRBox")) {
          $more_after = (new PageBreakLocator())->_getMoreAfter($i, $box->content, $content_size);
        }

        $locations = array_merge($locations, (new PageBreakLocator())->_getPagesTraverseInline($child, $penalty, $more_before, $more_after));
      } elseif (is_a($child, "TableRowBox")) {
        $locations = array_merge($locations, (new PageBreakLocator())->_getPagesTraverseTableRow($child, $penalty));
      }
    }
