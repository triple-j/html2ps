...
$g_media = (new Media())->predefined($g_config['media']);
...
// Setup watermark
$watermark_html = isset($_REQUEST['watermarkhtml']) ? $_REQUEST['watermarkhtml'] : "";
$pipeline->output_driver->set_watermark($watermark_html);
...
if ($_REQUEST['process_mode'] == 'batch') {
  ...
  $status = $pipeline->process($g_baseurl, $g_media);exit("5");
  exit('hallo welt11');
  exit("6");
}
